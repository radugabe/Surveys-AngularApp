...
@Component({
  selector: 'results-pie-chart',
  templateUrl: './results-chart-template-component.html',
  standalone: false
})
export class ResultsPieChartComponent implements OnChanges {
  @Input() survey!: Survey;
  @Input() aggregatedResults!: { [index: number]: { [option: string]: number } };

  charts: ChartConfiguration<'pie'>[] = [];

  ngOnChanges(changes: SimpleChanges): void {
    if (this.survey && this.aggregatedResults) {
      this.charts = this.generatePieCharts();
    }
  }

  private generatePieCharts(): ChartConfiguration<'pie'>[] {
